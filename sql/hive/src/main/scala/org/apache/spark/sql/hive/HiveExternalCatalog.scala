...
private[spark] class HiveExternalCatalog(conf: SparkConf, hadoopConf: Configuration)
   ...
   */
  override def alterDatabase(dbDefinition: CatalogDatabase): Unit = withClient("alterDatabase") {
   ...
   */
  override def alterTable(tableDefinition: CatalogTable): Unit = withClient("alterTable") {
    assert(tableDefinition.identifier.database.isDefined)
    val db = tableDefinition.identifier.database.get
  ...
  override def dropFunction(
      db: String,
      name: String): Unit = withClient("dropFunction") {
    requireFunctionExists(db, name)
    client.dropFunction(db, name)
  }

  override def alterFunction(
      db: String, funcDefinition: CatalogFunction): Unit = withClient("alterFunction") {
    requireDbExists(db)
    val functionName = funcDefinition.identifier.funcName.toLowerCase(Locale.ROOT)
